...
# 값 타입 
- JPA에서 데이터 타입을 가장 크게 분류하면 엔티티 타입과 값 타입을 나눈다.
- 엔티티 타입은 @Entity로 정의 값 타입은 int Integer String 처럼 단순한 값으로 사용하는 자바 기본 타입이나 객체
...
- 기본 생성자가 필수다. 
- 모든 값 타입은 엔티티의 생명주기에 의존하므로 엔티티와 임베디드 타입의 관계를 UML로 표현하면 컴포지션 관계가된다.

# 값 타입 컬렉션 
- 하나 이상 저장하려면 컬렉션에 보관하고 @ElementCollection, @Collection 어노테이션을 사용하면된다. 
- 식별자가 없다.
- 생명 주기를 엔티티에 의존한다.
